...
class AllExpensesController: UIViewController, UITableViewDelegate{
    ...
    func loadMessages() {
        ...
        let currDateString = formatter.string(from: Constants.date)
            ...
            if let e = error {
                ...
                if let snapshotDocuments = querySnapshot?.documents {
                    ...
                    for doc in snapshotDocuments {
                        var data = doc.data()
                        if data["user"] as! String == (Auth.auth().currentUser?.email)! && data["currDate"] as! String==currDateString{
                            let newMessage = Expense(amount: data["amount"] as! Double, category: data["category"] as! String, date: dateString, iden: data["iden"] as? String ?? NSUUID().uuidString, user: Auth.auth().currentUser?.email ?? "random@email.com" , currDate: currDateString)
